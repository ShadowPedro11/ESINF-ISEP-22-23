...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    final private Map<V, graph.map.MapVertex<V, E>> mapVertices;  // all the Vertices of the graph
    ...
    public Collection<Edge<V, E>> edges() {
        ...
        for (graph.map.MapVertex<V, E> mv : mapVertices.values())
    ...
    public Edge<V, E> edge(V vOrig, V vDest) {
        ...
        graph.map.MapVertex<V, E> mv = mapVertices.get(vOrig);
    ...
    public int outDegree(V vert) {
        ...
        graph.map.MapVertex<V, E> mv = mapVertices.get(vert);
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        ArrayList <Edge<V, E>> edgeArrayList = new ArrayList<>();
        for (V c: mapVertices.keySet()){
            Edge<V,E> edge = edge(c,vert);
            if(edge!=null){
    ...
    public boolean addVertex(V vert) {
        ...
        graph.map.MapVertex<V, E> mv = new graph.map.MapVertex<>(vert);
        vertices.add(vert);
    ...
    public boolean addEdge(V vOrig, V vDest, E weight) {
        ...
        graph.map.MapVertex<V, E> mvo = mapVertices.get(vOrig);
        graph.map.MapVertex<V, E> mvd = mapVertices.get(vDest);
